...
const NewTrain = () => {
  const navigate=useNavigate()
    ...
    const createNewTrain=()=>{
        setTrain({...train,intermediateStation,seatings,"availableDays":checkboxes})
        api.post("/train/new",{...train,intermediateStation,seatings,"availableDays":checkboxes}).then((res)=>{
          console.log(res)
        }).catch((err)=>console.log(err))
        navigate('/train/all')
        console.log({...train,intermediateStation,seatings,"availableDays":checkboxes})
    }
  ...
  return (
    <div className='container p-4 mt-28'>
      ...
      <button className='px-4 py-2 text-white rounded-md bg-blue-500' onClick={createNewTrain}>Submit</button>
    </div>
